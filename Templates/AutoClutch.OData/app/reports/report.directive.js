...
(function () {
    ...
    function reportController($scope, $log, $window, $http, $sce, $routeParams, reportService) {
        ...
        function activate() {
            ...
            getReport(vm.reportServerName, vm.reportPath, vm.reportName, vm.format, vm.parameters);
        }
        ...
        function getReport(reportServerName, reportPath, reportName, format, parameters) {
            if (format === 'pdf') {
                return reportService.getPdfReport(reportServerName, reportPath, reportName, format, parameters).then(function (response) {
                ...
                return reportService.getPdfReport(reportServerName, reportPath, reportName, format, parameters).then(function (response) {
                    ...
                    getReport(vm.reportServerName, vm.reportPath, reportName, vm.format, vm.parameters);
                });
            }
            else {
                return reportService.getHtmlReport(reportServerName, reportPath, reportName, format, parameters).then(function (response) {
